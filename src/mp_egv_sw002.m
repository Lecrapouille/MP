function [eigenvector, eigenvalue] = mp_egv_sw002(A, x0, r)
   ...
   %       "Power alghoritms for (max,+)- and (min,max,+)-systems",
   %       DEDS, vol.10, pp.369-389, 2000.
   %
   %   See also
   %   MP_EGV_BO93, MP_EGV_O91, MP_EGV_PQC, MP_EGV_SW002,
   %   MP_IS_EGV1, MP_IS_EGV2, MP_IS_PGA, MP_IS_PGSC1, MP_IS_PGSC1,
   ...
   sizeA = size(A);
   if (sizeA(1) ~= sizeA(2))
      error('mp_egv_sw002: algorithm works for square matrix A only!');
   end
   
   sizex =size(x0);
   if (sizex(2) ~= 1)
      error('mp_egv_sw002: x0 must be a column vector!');
   end
   
   if (sizex(1) ~= sizeA(1))
      error('mp_egv_sw002: inconsistent matrices sizes');
   end
   
   if (x0 == mp_zeros(sizex))
      error('mp_egv_sw002: for this x0 algorithm does not converge!');
   end
   ...
   if mp_is_egv1(A, eigenvector, eigenvalue) == 0
      ...
      error('mp_egv_sw002: number of iteration has exceed R_MAX!');
   end
% end of file